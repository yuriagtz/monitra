...
import { getLandingPagesByUserId } from './db';
  ...
  const task = cron.schedule(SCHEDULE_CHECK_TIME, async () => {
    try {
      ...
      try {
        ...
        // ユーザーの全LPを取得
        const allLps = await getLandingPagesByUserId(currentSetting.userId);
        
        // 除外LPをフィルタリング
        let targetLps = allLps;
        if (currentSetting.excludedLandingPageIds) {
          try {
            const excludedIds = JSON.parse(currentSetting.excludedLandingPageIds) as number[];
            targetLps = allLps.filter(lp => !excludedIds.includes(lp.id));
          } catch (error) {
        ...
        if (targetLps.length === 0) {
          console.log(`[Scheduler] No LPs to monitor for user ${currentSetting.userId} (${allLps.length} total, ${allLps.length - targetLps.length} excluded)`);
          // 次回実行予定日時を更新
          const nextRunAt = new Date(nowLocal);
          nextRunAt.setDate(nextRunAt.getDate() + currentSetting.intervalDays);
          nextRunAt.setHours(executeHour, 0, 0, 0);
        ...
        console.log(`[Scheduler] Running scheduled monitoring for user ${currentSetting.userId} (${targetLps.length} LP(s), ${allLps.length - targetLps.length} excluded)`);
        
        // 対象LPに対して監視を実行（並列実行、ただし重複実行を防ぐ）
        const monitoringPromises = targetLps.map(lp => {
          // LP単位での重複実行を防ぐ
          if (runningLps.has(lp.id)) {
            console.log(`[Scheduler] LP ${lp.id} is already being monitored, skipping duplicate`);
            return Promise.resolve(null);
          }
          
          // 実行中フラグを設定
          runningLps.add(lp.id);
          
          return monitorLandingPage(lp.id)
            .then(result => {
              runningLps.delete(lp.id);
              return result;
            })
            .catch(error => {
              runningLps.delete(lp.id);
              console.error(`[Scheduler] Error monitoring LP ${lp.id}:`, error);
              return null;
            });
        });
        ...
        console.log(`[Scheduler] Completed scheduled monitoring for user ${currentSetting.userId} (${targetLps.length} LP(s))`);
      } finally {
        // 実行中フラグを解除
...
export async function checkAndRunSchedules() {
  ...
  for (const schedule of schedules) {
    ...
    try {
      ...
      // ユーザーの全LPを取得
      const allLps = await getLandingPagesByUserId(schedule.userId);
      ...
      console.log(`[Scheduler] Running scheduled monitoring for user ${schedule.userId} (${targetLps.length} LP(s))`);
      ...
      console.log(`[Scheduler] Completed scheduled monitoring for user ${schedule.userId} (${targetLps.length} LP(s))`);
      executedCount++;
    } catch (error) {
      console.error(`[Scheduler] Error running scheduled task for schedule ${scheduleId}:`, error);
    } finally {
  ...
  console.log(`[Scheduler] Checked ${schedules.length} schedule(s), executed ${executedCount} schedule(s)`);
  return { checked: schedules.length, executed: executedCount };
}
