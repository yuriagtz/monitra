...
- `PUPPETEER_CHROME_BUILD_ID`: ChromeのビルドID（**削除推奨**）
  - **この環境変数は設定しないでください**
  - 設定している場合は、Vercelの環境変数設定から削除してください
  - この環境変数を設定すると、ライブラリ側の自動ビルドID解決（`getLastKnownGoodReleaseForChannel()`）が動作せず、存在しないビルドIDを指定してエラーになる可能性があります
  - 削除することで、ライブラリが「ちゃんと存在するビルドID」を自動解決し、Chromeのインストールが正常に動作します
